// ! SE REMOVIO LOGICA DE ECS6 POR QUE LA LIB DATABLES NO SOPRTA MODULOS
// import {
//   $$,
...
const $ContainerButton = $$(".loal-container-button-flotante-planta");
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (plantaGuardada) {
    ...
    await obtenerRecorridosPorPlanta({ id_planta: plantaGuardada });
      ...
      buttons: [
        ...
        //'pdf',
        {
          ...
          exportOptions: {
            ...
            format: {
              body: function (data, row, column, node) {
                // Si las columnas son las de imágenes (2 o 3), extrae el src de la imagen y lo convierte en un link
                if (column === 2 || column === 3) {
                  // Crea un contenedor temporal para convertir el string en HTML
                  // Crea un contenedor temporal para convertir el string en HTML
                  ...
                  // Extrae el atributo currentSrc para obtener la URL completa
                  var currentSrc = img.prop("currentSrc");

                  console.log("Imagen:", img);
                  console.log("currentSrc:", currentSrc);

                  // Devuelve el link o un texto alternativo si no hay imagen
                  return currentSrc 
                        ? 'Ver imagen (' + currentSrc + ')' // Se verá como texto en este paso
                        : 'No hay imagen';
                }

                // Para el resto de las columnas, elimina etiquetas HTML y deja solo texto
                return $("<div>").html(data).text(); // Elimina cualquier etiqueta HTML y deja el texto plano
              },
            },
          },
          customize: function (doc) {
            ...
            // Cambiar el tamaño de la página a 'A3' para hacerla más ancha (puedes ajustar a otros tamaños)
            doc.pageSize = 'A3';
    
            // Ajustar los márgenes (puedes modificar estos valores para hacer el PDF más ancho)
            doc.pageMargins = [10, 10, 10, 10];  // [izquierda, arriba, derecha, abajo]
    
            // Cambiar el estilo de la tabla si es necesario
            doc.defaultStyle.fontSize = 10;  // Ajusta el tamaño de la fuente si lo necesitas
    
            // Cambiar la alineación de la tabla
            doc.styles.tableHeader.alignment = 'center';  // Alinear el encabezado de la tabla al centro
            doc.styles.tableBodyEven.alignment = 'center'; // Alinear las filas pares al centro
            doc.styles.tableBodyOdd.alignment = 'center';  // Alinear las filas impares al centro
        },
  ...
  $btnRegistrarLP.addEventListener("click", () =>
    showModal($modalRegistrarRecorrido)
  );
...
// ! EXPORTS ,SE DEBERA PONER AQUI SIN USO DE MODULE POR QUE DATATABLES NO SOPORTA ACHVIOS MODULARES (TRI###)
 ...
 */
const obtenerDatosLinePatrol = async () => {
...
const contenedorDatos = (e) => {
  ...
  if (tagNameClick === "img") {
    ...
    if (viewportWidth <= 600) {
      ...
      let comentario = containerItems.querySelector(
        ".card-body > .card-text"
      ).textContent;
      ...
      let comentario =
        containerItems.querySelector(".card-text-small").textContent;
 ...
 */
const submitFiltrarDatosLinePatrol = async (e) => {
...
const corregirHallazgo = async (e) => {
  e.preventDefault();
  const formData = new FormData(e.target);
  ...
  try {
    ...
    if (response.ok) {
      ...
      e.target.reset();
      $$(
        "#exampleModalCorregirHallazgo .modal-dialog .modal-content .modal-header .btn-close"
      ).click();
 ...
 * @param {Event} e - EL EVENTO DE ENVÍO DEL FORMULARIO.
 */
const liberarLinePatrol = async (e) => {
  e.preventDefault(); // CANCELA LA ACCIÓN POR DEFECTO DEL FORMULARIO (RELOADING DE PÁGINA)
...
const showAndHideButtonFlotantePlantaRecorrido = () => {
 ...
 */
const confirmarPlantaRecorrido = async () => {
 ...
 */
const obtenerRecorridosPorPlanta = async ({ id_planta }) => {
  try {
    // MOSTRAR ANIMACIÓN DE CARGA
    $$(".container-loading").style.display = "flex";
